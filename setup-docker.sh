...
log_step "Добавляем GPG-ключ репозитория Docker" "bash -c \"curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add -\"" true
...
log_step "Проверка статуса Docker" "sudo systemctl status docker --no-pager" true
